...
<body class="h-full flex">
  ...
  ?>
  <div class="bg-gray-900 fixed w-full md:w-9/12 p-0 m-0 md:right-0 h-full flex justify-center flex-col items-center" id="home-div">
    ...
    <dialog id="postReport" class="modal bg-base-100">
      ...
      <form method="dialog">
        ...
        <label class="label">
          <span id="Post-Name" class="label-text">Porque deseja reportar: Nome do Post</span>
        </label>
        <input type="text" value="" id="post-reason" class="input input-bordered w-full bg-base-200" placeholder="Insira o motivo do report">
        </div>
        <div class="form-control mt-4">
        <label class="label">
        ...
        <select id="postType" class="select select-bordered w-full bg-base-200">
          ...
          <option value="type2">Não está relacionado com o tema</option>
          <option value="type3">Plágio</option>
        </select>
        </div>
        <div class="form-control mt-4 flex flex-row gap-2">
        <button class="btn flex-initial text-orange-500" onclick="saveReport()">Reportar</button>
